...
export const BRAND_QUERY = `{
  ...
  "brand": *[_type == "brand" && slug.current == $slug][0]{
    ...
    seoTitle, seoDescription,
    heroHeading, heroIntro, heroTicks, heroImage, heroCtaLabel, quoteHeading, quoteText,
    spacious,
    sections[]{
      _type, _key, heading, eyebrow, intro, note, ctaLabel, ctaHref, tone, anchor,
      image, imagePath, imageAlt,
      points[]{ title, text },
      specs[]{ label, value },
      compareColumns, compareRows[]{ label, values },
      text
    },
    certifiedHeading, certifiedText, certifiedBadge, certifiedImage, certifiedImagePath, certifiedPoints,
    ...
    workHeading, workSlug,
    areasHeading, areasIntro,
    relatedHeading, relatedIntro, relatedServices[]->{ title, "slug": slug.current, summary, icon },
    faqHeading, faqNote, faqs[]{ question, answer },
    ctaEyebrow, ctaHeading, ctaText
  },
  "navBrands": *[_type == "brand"] | order(order asc, name asc){ _id, name, kind, hasPage, "slug": slug.current },
  "services": *[_type == "service"] | order(order asc){ _id, title, slug, summary, icon, hasPage },
