...
| `Revelectrical-search-report-19-Aug-2026.pdf` | The same report, one continuous page |
...
The PDF is a single continuous page, not A4 sheets: the height is measured
from the document after print styles are applied, so nothing is split across a
break. Width stays at A4's 794px so the three stat cards hold one row and the
type keeps a sensible measure.

It also forces the light palette regardless of the machine's theme, opens the
methodology disclosure (nothing is clickable on paper), and uses zero page
margins because the masthead and footer are full-bleed bands - the print
