...
const SITE = path.resolve(HERE, '../../../revelectrical-site');
const OUT = path.resolve(HERE, '../ev-charging-social-posts.html');

const b64 = (file, mime) =>
  `data:${mime};base64,${fs.readFileSync(file).toString('base64')}`;

const font = (fam, w) =>
  b64(path.join(HERE, 'fonts', `${fam}-latin-${w}-normal.woff2`), 'font/woff2');
...
const MAP = {
  ...
  __IMG_LOGO__: b64(path.join(SITE, 'public/logo-light.webp'), 'image/webp'),
  __IMG_LOGO_DARK__: b64(path.join(SITE, 'public/logo.webp'), 'image/webp'),
  __SVG_TICK__: TICK,
